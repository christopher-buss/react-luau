...
| `packages/react-reconciler/src/ReactFiberErrorLogger.js` defaults, `logUncaughtError`, `logCaughtError` | `modules/react-reconciler/src/ReactFiberErrorLogger.lua` | Adapted | Roblox has no browser `reportError`, environment-badged console binding, console task integration, or React 19 Act queue. Component stacks and class Error Boundary instances are preserved. React 17's caught-error message shape remains the internal-renderer console contract. |
| `packages/shared/reportGlobalError.js` | `modules/shared/src/reportGlobalError.lua`, `init.lua` | Adapted | A deferred thread error is Roblox's global uncaught-reporting boundary. `errorToString` formats values for the top-level VM handler; errors thrown by root callbacks keep their original value. |
...
| `packages/react-reconciler/src/ReactFiberWorkLoop.js` retry, commit, and commit-phase capture | `modules/react-reconciler/src/ReactFiberWorkLoop.new.lua` | Adapted | React 17 lanes and recursive commit traversal are retained. First-attempt captured errors become recoverable only after a successful synchronous retry. The recovery collections use the predeclared module table to keep this near-limit Luau chunk loadable. React 17 callers perform hydration fallback before the helper; uncached-thenable ping detection and second-attempt recoverable queue merging are omitted because this work loop has none of the corresponding React 19 state. |
| `packages/react-noop-renderer/src/createReactNoop.js` `createRoot` options | `modules/react-noop-renderer/src/createReactNoop.lua` | Adapted | Transition callbacks and default transition indicators are unavailable. Internal default roots retain React 17's synchronous throw and console output; explicit error handlers use the React 19 contract. Unlike upstream noop, React-Luau honors `onRecoverableError`. |
| `packages/react-reconciler/src/__tests__/ReactConfigurableErrorLogging-test.js` | `modules/react-roblox/src/client/__tests__/ReactRobloxConfigurableErrorLogging.spec.lua` | Adapted | The five renderer-observable upstream cases retain their order and assertions through `ReactRoblox.createRoot`. A focused ReactNoop regression preserves the legacy internal-renderer console and throw contract. Roblox Instances replace DOM nodes and Scheduler flushing replaces async fake Act. |
...
| `packages/react-test-renderer/src/ReactTestRenderer.js` and its internal test | Existing renderer calls into `createContainer` defaults | Adapted | React Test Renderer has no public root option surface here and retains React 17's synchronous throw and console-reporting contract. |
