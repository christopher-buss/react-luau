...
It stacks on the transition lanes in Roblox/react-luau#25, root error reporting
in Roblox/react-luau#31, and the generic thenable substrate in
Roblox/react-luau#32, in that order.
...
Roblox/react-luau#33 is a sibling, not a merge-order dependency, but both
branches independently carry the same
...
| `ReactFiberHooks.js` Action-state queue | `ReactFiberHooks.new.lua` | Adapted | Action nodes expose `andThen` and read through Roblox/react-luau#32's canonical `useThenable` and `SuspenseException`. React 17 traverses the queue and transition hooks before a read can suspend because it has no suspended-component replay dispatcher. |
| `ReactFiberThenable.js` generic thenable state and suspension sentinel | `modules/react-reconciler/src/ReactFiberThenable.lua` | Dependency | Roblox/react-luau#32 supplies the single thenable reader and identity-stable suspension sentinel used by both `use` and Actions. Actions adds no private generic reader or sentinel. |
| `ReactFiberThrow.js` concurrent suspension without a boundary | `ReactFiberThrow.new.lua`, `ReactFiberUnwindWork.new.lua`, and `ReactFiberWorkLoop.new.lua` | Adapted | React 19's root suspension path restores an existing interrupted component, accepts wakeables thrown by React 17 HostRoot Action queues, unwinds without an error-capture pass, and keeps the current UI while an Action transition is pending. Sentinel conversion comes from Roblox/react-luau#32, and the generic root path overlaps Roblox/react-luau#33. |
...
| 14 | `useOptimistic rebases if the passthrough is updated during a render phase update (initial mount)` | Adapted | ReactNoop host output and a controlled wakeable replace JSX and the JavaScript Promise while preserving the initial-mount rebase. |
...
| 17 | `optimistic state is not reverted until async action finishes, even if useTransition hook is unmounted` | Adapted | Sibling host nodes are flattened. A `false` child preserves upstream's empty slot and sibling identity because Luau varargs drop conditional `nil` children. |
| 18 | `updates in an async action are entangled even if useTransition hook is unmounted before it finishes` | Adapted | ReactNoop text replaces span nodes, and a `false` child preserves the sibling's position across owner removal. |
| 19 | `updates in an async action are entangled even if useTransition hook is unmounted before it finishes (class component)` | Adapted | React-Luau class syntax and a positional `false` child preserve the class sibling across owner removal. |
| 20 | `updates in an async action are entangled even if useTransition hook is unmounted before it finishes (root update)` | Adapted | ReactNoop text replaces span nodes, and a `false` child preserves the text position across owner removal. |
...
| 24 | `React.startTransition captures async errors and passes them to reportError` | Adapted | The suite replaces Shared `reportGlobalError` with a scheduler reporter, verifies no early report, then rejects the pending Action. |
| 25 | `React.startTransition captures sync errors and passes them to reportError` | Adapted | The same scoped Shared reporter captures the synchronous callback failure. |

## Action-state test ledger

The 13 renderer-independent `useActionState` cases from
[`ReactDOMForm-test.js`](https://github.com/facebook/react/blob/ae74234eae6ebd62f19190731278e20bc1c37d51/packages/react-dom/src/__tests__/ReactDOMForm-test.js#L980-L1508)
...
| 2 | `useActionState supports inline actions` | Adapted | ReactNoop replaces the DOM root while the synchronous Action and implementation replacement contract stay unchanged. |
| 3 | `useActionState: dispatch throws if called during render` | Adapted | ReactNoop replaces the DOM root while preserving the render-time dispatch and exact error. |
...
| 6 | `useActionState: works if action is sync` | Adapted | ReactNoop replaces the DOM root while preserving the synchronous state and pending sequence. |
...
19 error contracts: global `React.startTransition` reports through Shared
`reportGlobalError`, while `useTransition` callback errors continue through the
renderer error path after transition context restoration. Debug Tools hook-slot
traversal and reconciler hook-order coverage are listed in the source ledger
above.

## Stack order and merge shrinkage

This integration branch contains the exact public heads of
Roblox/react-luau#25, Roblox/react-luau#31, and Roblox/react-luau#32 before its
Actions-specific integration commits. Runtime verification therefore exercises
the real transition, global-error, and thenable prerequisites rather than
temporary adapters. After each dependency lands, rebase this branch onto
`main` and drop that dependency's commits from the pull-request diff; the
Actions code keeps using the same canonical seams.

Roblox/react-luau#33 remains a sibling. Whichever sibling lands second drops
its duplicate `ReactFiberThrow`, Work Loop suspension callback, and HostRoot
capture hunks while preserving both additive HostRoot begin-work behaviors.
The final graph contains one concurrent-root suspension substrate and no
duplicate thenable or global-error implementation.
